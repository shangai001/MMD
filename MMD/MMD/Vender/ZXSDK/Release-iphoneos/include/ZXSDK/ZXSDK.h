 ...
 */
typedef enum {
    ZXReason_UNMATCH = 0,         //01：证件照和真人不匹配；
    ZXReason_NOTCLEAR,            //02：拍照不清晰；
    ZXReason_FACK,                //03：疑似有假；
    ZXReason_NOTVALID,            //04：无法验证；
    ZXReason_OTHER                //99：其它原因
} ZXUnbindReasonCode;
 ...
 *
 * @param uid :用户在第三方APP体系下的id值
 * @param reasonCode :解绑原因代码
 * @param reasonRemark :解绑原因说明,如果reasonCode为ZXReason_OTHER（其它原因），则可以在这里给出说明。
 * @param callback :回调函数
 *
 * @return taskId :任务id
 */
+(NSUInteger)unbindForUid:(NSString *)uid forReasonCode:(ZXUnbindReasonCode)reasonCode forReasonRemark:(NSString *)reasonRemark withCallback:(ZXCallback)callback;
